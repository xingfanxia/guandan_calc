...
- **Visual testing (rebuilt 2026-06-12)**: `npm run test:visual` is the LOCAL pixel gate — pixelmatch against 21 baseline PNGs (`docs/reports/redesign/` 19: 4 pages × light/dark × mobile/desktop incl. ranking/session/victory states; `docs/reports/png-export/` 2 canvas exports). Capture scripts: `capture-redesign.mjs` + `capture-png-exports.mjs`, deterministic via `scripts/visual/_fixtures.mjs` (`freezeTime` + `setDeterministicPlayers`) + route-mocked APIs (pathname predicate, NOT a `**/api/**` glob — the glob would intercept `/src/api/*.js` module loads and break Vite). Default threshold 100 px; `png-export/` overridden to 250 px in `diff-baselines.mjs` for canvas font subpixel noise. `scripts/visual/test-theme-toggle.mjs` adds 14 light/dark assertions. CI (`.github/workflows/visual-regression.yml`) runs the toggle smoke + capture suite as a STRUCTURAL check (zero page errors + full 21-PNG set) — pixel comparison is local-only because the system font stack renders differently across OSes. Capture scripts honor `GD_BASE_URL` when port 3000 is taken.
