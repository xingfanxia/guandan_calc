...
| **2.5** | Linear sidebar layout via `layout.mount()` + state preservation across theme switches. | Validates the layout mount/unmount path of the manager beyond CSS-only restyles. | **SHIPPED 2026-05-05** — `themeManager.mount()` now calls `current.layout?.unmount()` before swapping (prevents DOM leak). `linear/index.js` `layout.mount()` moves the live `<nav class="topnav">` into a new `<aside class="linear-sidebar">` wrapper (move-not-clone preserves event listeners + `#themePickerMount` slot); `layout.unmount()` restores topnav to its original parent at original next-sibling position. CSS-only `display: contents` mobile fallback so the @media (max-width:768px) view reverts to the default sticky-top row layout — no JS resize listener needed. State preservation is structural: `state.js` is a module-scope singleton; theme.layout only mutates DOM, never touches state, so swap is automatically lossless. The architecture-doc `getSnapshot/restore` pseudocode in Section 3 turned out to be illustrative — the singleton + localStorage persist provides preservation for free. 20-assertion smoke test at `scripts/visual/test-theme-switch.mjs` verifies all 4 invariants (mount, unmount, remount, state-survival across full 4-theme cycle). VR baselines regenerated for Linear desktop. **Phase 2.6 polish landed 2026-05-06** (see next row). |
| **2.6** | Linear sidebar polish — visual hierarchy details that bring the live sidebar closer to `demos/demo-linear-v2.png`. | Capture the demo's section-label / accent-bar / status-dot affordances without fabricating IA the app doesn't actually have. | **SHIPPED 2026-05-06** (`a92576d`) — `linear/index.js` `layout.mount()` now also injects two transient nodes after the topnav move: `<div class="linear-sidebar__section-label">导航 NAVIGATION</div>` before `.topnav__tabs` (uppercase mono, ink-dimmer, letter-spaced), and `<span class="linear-sidebar__status-dot">` inside the user avatar (small green online indicator). `layout.unmount()` strips these BEFORE restoring topnav so the original DOM is byte-identical to pre-mount state. Active-tab accent bar is pure CSS (`::before` on `.topnav__tab--active`, 2px Linear-purple vertical line on the left edge — no DOM mutation needed). Mobile (<769px) hides both injected nodes via `display: none` so the Phase 2.5 `display: contents` fallback still produces a clean topnav row. Decorative ⌘1/⌘2/⌘3 shortcut chips were considered and rejected — Cmd+1/2/3 are browser-reserved, so non-functional chips would be aspirational lying. 20-assertion smoke test still passes (mount + unmount + remount + 4-theme cycle). VR baselines regenerated for Linear desktop where the sidebar polish is visible. |
...
| **5** | Add Tea-Table last — needs custom illustrations (ink portraits, brushstroke SVGs) which is the most expensive asset work. | Don't gate the rollout on an asset commission | **SHIPPED 2026-05-06** (`6454326` + `1d09538`) — fifth registered theme. Aesthetic: contemplative scholar's-table at dusk. Deep warm-graphite oklch (60° hue) surfaces, vermillion seal accent at hue 25°, Noto Serif SC + Noto Sans SC, generous letter-spacing for editorial CJK rhythm. **The asset commission turned out to be unnecessary** — gpt-image-2 (Azure) generates traditional Chinese ink-wash portraits at production quality, so all 17 portraits (16 honor archetypes + 1 profile fallback) were AI-generated rather than commissioned. Stored at `public/themes/teatable/honors/<honor-id>.jpg`. Tea-Table is the first theme to consume the previously-declarative `featureManifest.honorPortraits === 'photo'` value: `src/stats/honors.js renderHonors()` reads it and injects `<img class="honor__portrait">` per article (idempotent, theme-switch-safe — strips on switch to a non-photo theme). Layout uses absolute-positioned portrait + flow body so CJK characters wrap normally; flex-on-article would vertically stack characters. VR coverage extended to `phase5-teatable/` (16 baselines) + 2 new cross-theme victory baselines. 65 → 83 baselines. 20-assertion smoke test grew to 21. Generation script: `scripts/teatable/generate-portraits.py`. **All five planned theme phases now shipped.** |
