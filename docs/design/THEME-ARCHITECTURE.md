...
| **4** | Add Atelier (warm-graphite, photographic moments) — different in palette family + asset needs. | Validates theme-specific assets (illustrations, photos) | **SHIPPED 2026-05-04** — `src/themes/atelier/{theme.css, featureManifest.js, index.js}` registered as fourth theme. CSS-only restyle (no commissioned photo assets in this iteration). Editorial magazine framing: warm graphite oklch 60° hue, clay/caramel accent at 65°, Fraunces serif + Inter + JetBrains Mono, vintage card-stock pcards, gold rule between sections. Visual baseline: `docs/reports/phase4-atelier/`. |
