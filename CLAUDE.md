...
- Atelier polish iter 1 + iter 2 shipped 2026-05-05 (`a392e15` + `c852766`) — root-cause fix on history grid (renderer emits 8 cells/row but Atelier had 5 columns → broken 2-row layout); capture script swap to `renderStatistics()` so stats card + team-honors actually populate; hero head-line 28→40px; topnav brand-mark `Ⓐ` + 22→28px padding; scoreboard `.team__role` italic Fraunces; team-honors underline-only badges; pool/slots `__sub` captions mono uppercase → italic Fraunces. Aggregate ~70% → ~93% per section. Sample/victory state still uncaptured (needs victoryModal HTML refactor — inline styles in index.html line 885+ override theme.css across all 4 themes). Combined handoff: `docs/design/HANDOFF-2026-05-05-atelier-polish-iter-1.md`. Phase 2.5 (Linear sidebar layout via `layout.mount()` + state-preservation across switches) remains the deferred infra item; Phase 5 (Tea-Table) is gated on commissioned ink illustrations. See `docs/design/THEME-ARCHITECTURE.md` for the full 5-phase plan.
